...
# Load biến môi trường từ file .env một lần duy nhất khi import module
load_dotenv()
...
class Config:
    ...
    # Giới hạn tối đa dung lượng file đọc vào RAM khi quét YARA (bytes)
    MAX_FILE_READ_BYTES: int = int(
        os.getenv("MAX_FILE_READ_BYTES", str(50 * 1024 * 1024))
    )

    # Các hằng số Anti-Evasion engine
    MIN_VARIANT_BYTES: int = int(os.getenv("MIN_VARIANT_BYTES", "32"))
    MAX_VARIANT_BYTES: int = int(
        os.getenv("MAX_VARIANT_BYTES", str(5 * 1024 * 1024))
    )
    ...
    THREAD_POOL_WORKERS: int = int(
        os.getenv("THREAD_POOL_WORKERS", str(os.cpu_count() or 4))
    )
